...
var app = function() {
  ...
  ioServer.regReqHdlr("write", function(args, cb) {
    cserver
      .find(args.permAddr)
      .writeReq(args.auxId, args.value, function(err, rsp) {
        if (err) cb(err);
        else cb(null);
      });
  });
  ...
  cserver.on("ready", function() {
    ...
    setImmediate(function() {
      cserver.permitJoin(Number.POSITIVE_INFINITY);
    });
  });
  ...
  var dbPath = path.resolve(
    __dirname,
    "../node_modules/coap-shepherd/lib/database/coap.db"
  );
...
function showWelcomeMsg() {
  var coapPart1 = chalk.blue(
      "   _____ ____   ___    ___          ____ __ __ ____ ___   __ __ ____ ___   ___ "
    ),
    coapPart2 = chalk.blue(
      "  / ___// __ \\ / _ |  / _ \\  ____  / __// // // __// _ \\ / // // __// _ \\ / _ \\"
    ),
    coapPart3 = chalk.blue(
      " / /__ / /_/ // __ | / ___/ /___/ _\\ \\ / _  // _/ / ___// _  // _/ / , _// // /"
    ),
    coapPart4 = chalk.blue(
      " \\___/ \\____//_/ |_|/_/          /___//_//_//___//_/   /_//_//___//_/|_|/____/ "
    );
  ...
  console.log(
    chalk.gray(
      "            An implementation of CoAP device management Server."
    )
  );
  console.log("");
...
function setLeaveMsg() {
  ...
  function showLeaveMessage() {
    console.log(" ");
    console.log(
      chalk.blue("      _____              __      __                  ")
    );
    console.log(
      chalk.blue("     / ___/ __  ___  ___/ /____ / /  __ __ ___       ")
    );
    console.log(
      chalk.blue("    / (_ // _ \\/ _ \\/ _  //___// _ \\/ // // -_)   ")
    );
    console.log(
      chalk.blue("    \\___/ \\___/\\___/\\_,_/     /_.__/\\_, / \\__/ ")
    );
    console.log(
      chalk.blue("                                   /___/             ")
    );
    ...
    console.log(
      "    >>>   " + chalk.yellow("https://github.com/PeterEB/coap-shepherd")
    );
    console.log(" ");
    process.exit();
  }
...
function devStatusInd(permAddr, status) {
  ...
  if (status === "online") status = chalk.green(status);
  else status = chalk.red(status);

  console.log(
    chalk.magenta("[     devStatus ] ") + "@" + permAddr + ", " + status
  );
}
